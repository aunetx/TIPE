...
def courant_x(dt, X, Y):
    return np.ones(n) * 0.7


def courant_y(dt, X, Y):
    return np.zeros(n)
...
for dt in range(1, T):
    ...
    marche_alea_x = np.zeros(n)
    marche_alea_x[:v] = np.random.randn(v)
    X[dt] = X[dt-1] + marche_alea_x

    marche_alea_y = np.zeros(n)
    marche_alea_y[:v] = np.random.randn(v)
    Y[dt] = Y[dt-1] + marche_alea_y

    for force in forces:
        (force_x, force_y) = force
        X[dt] = X[dt] + force_x(dt, X[dt], Y[dt])
        Y[dt] = Y[dt] + force_y(dt, X[dt], Y[dt])
